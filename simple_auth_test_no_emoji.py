...
if register_resp:
    ...
    if register_resp.status_code == 401 and "Authorization header is required" in str(register_resp.text):
        print("   ERROR: Register endpoint incorrectly requires auth header")
    else:
        print("   SUCCESS: Register endpoint is accessible without auth header")
...
if login_resp:
    ...
    if login_resp.status_code == 401 and "Authorization header is required" in str(login_resp.text):
        print("   ERROR: Login endpoint incorrectly requires auth header")
    else:
        print("   SUCCESS: Login endpoint is accessible without auth header")

# Test if there's an error about auth header specifically
print("\n4. Checking for specific auth header errors:")
if register_resp and "Authorization header is required" in str(register_resp.text):
    print("   ERROR: Register endpoint incorrectly requires auth header")
elif login_resp and "Authorization header is required" in str(login_resp.text):
    print("   ERROR: Login endpoint incorrectly requires auth header")
else:
    print("   SUCCESS: No auth header requirement errors found")
