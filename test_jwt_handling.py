...
def test_jwt_handling():
    """Test that JWT tokens are properly handled between frontend and backend"""
    ...
    # 2. Decode the JWT to check its structure
    try:
    ...
    # 3. Test using the token to access protected resources
    ...
    # 4. Check that the token can be used with the task endpoints (which require authentication)
    ...
    # 5. Verify that token validation fails with an invalid token
    ...
    print("\n[SUCCESS] JWT token handling test completed successfully!")
    print("\nSummary:")
    print("- Tokens are properly generated upon registration/login")
